...
export class GoogleDriveManager {
  private storageKey = "classmate_gdrive_script_url";
  private scriptUrl: string;

  constructor() {
    this.scriptUrl = typeof window !== "undefined" ? localStorage.getItem(this.storageKey) || "" : "";
  }
  ...
  setScriptUrl(url: string): boolean {
  ...
  async uploadFile(file: File, folderName = "ClassMate_Practicum_Files"): Promise<string> {
    const dataUrl = await this.readFileAsDataURL(file);

    if (!this.isConfigured()) {
      // If Google Drive is not configured, fallback to local Data URL
      return dataUrl;
    }
  ...
  async syncPortfolioToCloud(studentId: string, portfolioData: any): Promise<boolean> {
  ...
  async fetchPortfolioFromCloud(studentId: string): Promise<any | null> {
    if (!this.isConfigured()) return null;

    try {
      const response = await fetch(
        `${this.scriptUrl}?action=getPortfolio&studentId=${encodeURIComponent(studentId)}`
      );
      const result = await response.json();
